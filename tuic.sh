...
if [ -d "/root/tuic" ]; then
    ...
    case $choice in
        ...
        2)   read -p $'\033[1;35mEnter a UUID (or press enter for a random UUID): \033[0m' new_uuid
            ...
            echo -e "\e[1;33m\nV2rayN、NekoBox\e[0m"
            echo -e "\e[1;32mtuic://$new_uuid:$password@$public_ip:$new_port?congestion_control=bbr&alpn=h3&sni=touch.10086.cn&udp_relay_mode=native&allow_insecure=1#$isp\e[0m"
            echo ""
            exit 0
            ;;
        3)
...
# nekoray/nekobox URL
echo -e "\e[1;33m\nV2rayN、NekoBox\e[0m"
echo -e "\e[1;32mtuic://$UUID:$password@$public_ip:$port?congestion_control=bbr&alpn=h3&sni=touch.10086.cn&udp_relay_mode=native&allow_insecure=1#$isp\e[0m"
echo ""
